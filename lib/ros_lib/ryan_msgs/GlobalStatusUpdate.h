#ifndef _ROS_ryan_msgs_GlobalStatusUpdate_h
#define _ROS_ryan_msgs_GlobalStatusUpdate_h

#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include "ros/msg.h"

namespace ryan_msgs
{

  class GlobalStatusUpdate : public ros::Msg
  {
    public:
      typedef int32_t _systemStatus_type;
      _systemStatus_type systemStatus;
      typedef int32_t _refId_type;
      _refId_type refId;

    GlobalStatusUpdate():
      systemStatus(0),
      refId(0)
    {
    }

    virtual int serialize(unsigned char *outbuffer) const
    {
      int offset = 0;
      union {
        int32_t real;
        uint32_t base;
      } u_systemStatus;
      u_systemStatus.real = this->systemStatus;
      *(outbuffer + offset + 0) = (u_systemStatus.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_systemStatus.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_systemStatus.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_systemStatus.base >> (8 * 3)) & 0xFF;
      offset += sizeof(this->systemStatus);
      union {
        int32_t real;
        uint32_t base;
      } u_refId;
      u_refId.real = this->refId;
      *(outbuffer + offset + 0) = (u_refId.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_refId.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_refId.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_refId.base >> (8 * 3)) & 0xFF;
      offset += sizeof(this->refId);
      return offset;
    }

    virtual int deserialize(unsigned char *inbuffer)
    {
      int offset = 0;
      union {
        int32_t real;
        uint32_t base;
      } u_systemStatus;
      u_systemStatus.base = 0;
      u_systemStatus.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_systemStatus.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_systemStatus.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_systemStatus.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->systemStatus = u_systemStatus.real;
      offset += sizeof(this->systemStatus);
      union {
        int32_t real;
        uint32_t base;
      } u_refId;
      u_refId.base = 0;
      u_refId.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_refId.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_refId.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_refId.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->refId = u_refId.real;
      offset += sizeof(this->refId);
     return offset;
    }

    const char * getType(){ return "ryan_msgs/GlobalStatusUpdate"; };
    const char * getMD5(){ return "e3da4c0373772ac9e1de0daa1ce01020"; };

  };

}
#endif
